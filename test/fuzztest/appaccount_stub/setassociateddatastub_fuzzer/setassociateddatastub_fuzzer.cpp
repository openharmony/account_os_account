...
#include "setassociateddatastub_fuzzer.h"
...
bool SetAssociatedDataStubFuzzTest(const uint8_t* data, size_t size)
{
    ...
    std::string value = fuzzData.GenerateRandomString();
    ...
    if (!dataTemp.WriteString(value)) {
        return false;
    }
    MessageParcel reply;
    MessageOption option;
    uint32_t code = static_cast<uint32_t>(AppAccountInterfaceCode::SET_ASSOCIATED_DATA);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::SetAssociatedDataStubFuzzTest(data, size);
    return 0;
}
