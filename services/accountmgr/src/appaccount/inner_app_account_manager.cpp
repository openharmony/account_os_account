    ...
    bool &isAccessible, const uid_t &uid, const std::string &bundleName)
{
    ...
    return controlManagerPtr_->CheckAppAccess(name, authorizedApp, isAccessible, uid, bundleName);
}
    ...
    const std::string &credential, const uid_t &uid, const std::string &bundleName)
{
    ...
    ErrCode result =
        controlManagerPtr_->SetAccountCredential(name, credentialType, credential, uid, bundleName);
    ...
    const uid_t &uid, const std::string &bundleName)
{
    ...
    ErrCode result = controlManagerPtr_->SelectAccountsByOptions(options, callback, uid, bundleName);
    return result;
}
