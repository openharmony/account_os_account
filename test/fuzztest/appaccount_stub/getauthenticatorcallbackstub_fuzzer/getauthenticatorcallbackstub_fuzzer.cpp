...
bool GetAuthenticatorCallbackStubFuzzTest(const uint8_t* data, size_t size)
{
    ...
    uint32_t code = static_cast<uint32_t>(IAppAccountIpcCode::COMMAND_GET_AUTHENTICATOR_CALLBACK);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::GetAuthenticatorCallbackStubFuzzTest(data, size);
    return 0;
}
