...
namespace OHOS {
    bool UnsubscribeAppAccountFuzzTest(const uint8_t* data, size_t size)
    {
        bool result = false;
        if (size > 0) {
            ...
            result = AppAccountManager::UnsubscribeAppAccount(subscriberTestPtr);
        }
        return result;
    }
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::UnsubscribeAppAccountFuzzTest(data, size);
    return 0;
}
