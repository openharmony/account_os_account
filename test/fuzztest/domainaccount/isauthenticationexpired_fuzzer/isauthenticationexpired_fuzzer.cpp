...
#include "isauthenticationexpired_fuzzer.h"
...
bool IsAuthenticationExpiredFuzzTest(const uint8_t* data, size_t size)
{
    bool ret = true;
    ...
    bool isExpired = fuzzData.GenerateBool();
    ret = DomainAccountClient::GetInstance().IsAuthenticationExpired(info, isExpired);
    return ret == ERR_OK;
}
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::IsAuthenticationExpiredFuzzTest(data, size);
    return 0;
}
