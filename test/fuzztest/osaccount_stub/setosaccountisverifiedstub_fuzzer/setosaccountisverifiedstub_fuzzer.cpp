...
#include "setosaccountisverifiedstub_fuzzer.h"
#include <string>
#include <thread>
#include <vector>
...
bool SetOsAccountIsVerifiedStubFuzzTest(const uint8_t *data, size_t size)
{
    ...
    osAccountManagerService_ ->OnRemoteRequest(
        static_cast<int32_t>(IOsAccountIpcCode::COMMAND_SET_OS_ACCOUNT_IS_VERIFIED), datas, reply, option);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::SetOsAccountIsVerifiedStubFuzzTest(data, size);
    return 0;
}
