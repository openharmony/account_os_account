 ...
 * @tc.desc: Open Session failed with no permission.
 * @tc.type: FUNC
...
HWTEST_F(AccountIAMClientTest, AccountIAMClient_AddCredential_0100, TestSize.Level0)
{
    ...
    AccountIAMClient::GetInstance().AddCredential(testPara, testCallback);
}
...
HWTEST_F(AccountIAMClientTest, AccountIAMClient_UpdateCredential_0100, TestSize.Level0)
{
    ...
    EXPECT_NE(testCallback, nullptr);
    EXPECT_CALL(*testCallback, OnResult(_, _)).Times(Exactly(1));
    AccountIAMClient::GetInstance().UpdateCredential(testPara, testCallback);
}
...
HWTEST_F(AccountIAMClientTest, AccountIAMClient_Cancel_0100, TestSize.Level0)
{
    ...
    int32_t status = 0;
    AccountIAMClient::GetInstance().Cancel(testChallenge, status);
    EXPECT_NE(status, 0);
}
...
HWTEST_F(AccountIAMClientTest, AccountIAMClient_DelCred_0100, TestSize.Level0)
{
    ...
    EXPECT_NE(testCallback, nullptr);
    EXPECT_CALL(*testCallback, OnResult(_, _)).Times(Exactly(1));
    AccountIAMClient::GetInstance().DelCred(testCredentialId, testAuthToken, testCallback);
}
...
HWTEST_F(AccountIAMClientTest, AccountIAMClient_DelUser_0100, TestSize.Level0)
{
    ...
    EXPECT_NE(testCallback, nullptr);
    EXPECT_CALL(*testCallback, OnResult(_, _)).Times(Exactly(1));
    AccountIAMClient::GetInstance().DelUser(testAuthToken, testCallback);
}
...
HWTEST_F(AccountIAMClientTest, AccountIAMClient_GetCredentialInfo_0100, TestSize.Level0)
{
    ...
    AccountIAMClient::GetInstance().GetAuthInfo(AuthType::PIN, testCallback);
}
...
HWTEST_F(AccountIAMClientTest, AccountIAMClient_GetProperty_0100, TestSize.Level0)
{
    ...
    auto testCallback = std::make_shared<MockGetPropCallback>();
    EXPECT_NE(testCallback, nullptr);
    EXPECT_CALL(*testCallback, OnResult(_, _)).Times(1);
    AccountIAMClient::GetInstance().GetProperty(testRequest, testCallback);
}
...
HWTEST_F(AccountIAMClientTest, AccountIAMClient_SetProperty_0100, TestSize.Level0)
{
    ...
    auto testCallback = std::make_shared<MockSetPropCallback>();
    EXPECT_NE(testCallback, nullptr);
    EXPECT_CALL(*testCallback, OnResult(_, _)).Times(1);
    AccountIAMClient::GetInstance().SetProperty(testRequest, testCallback);
}
...
{
    SetPropertyRequest testRequest = {};
    auto testCallback = std::make_shared<MockAuthenticationCallback>();
    EXPECT_NE(testCallback, nullptr);
    EXPECT_CALL(*testCallback, OnResult(_, _)).Times(1);
    uint64_t contextId = 0;
    AccountIAMClient::GetInstance().AuthUser(
        TEST_USER_ID, TEST_CHALLENGE, AuthType::PIN, AuthTrustLevel::ATL1, testCallback, contextId);
}
...
HWTEST_F(AccountIAMClientTest, AccountIAMClient_Auth_0100, TestSize.Level0)
{
    ...
    EXPECT_CALL(*testCallback, OnResult(_, _)).Times(1);
    uint64_t contextId = 0;
    AccountIAMClient::GetInstance().Auth(TEST_CHALLENGE, AuthType::PIN, AuthTrustLevel::ATL1, testCallback, contextId);
}
...
{
    int32_t ret = 0;
    AccountIAMClient::GetInstance().CancelAuth(TEST_CONTEXT_ID, ret);
    EXPECT_NE(ret, 0);
}
}  // namespace AccountTest
}  // namespace OHOS