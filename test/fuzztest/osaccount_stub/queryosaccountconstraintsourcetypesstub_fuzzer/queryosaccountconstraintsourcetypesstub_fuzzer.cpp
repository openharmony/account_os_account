...
#include "queryosaccountconstraintsourcetypesstub_fuzzer.h"
#include <string>
#include <thread>
#include <vector>
...
bool QueryOsAccountConstraintSourceTypesStubFuzzTest(const uint8_t *data, size_t size)
{
    ...
    std::string constraint(reinterpret_cast<const char*>(data), size);
    if (!datas.WriteString(constraint)) {
        return false;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::QueryOsAccountConstraintSourceTypesStubFuzzTest(data, size);
    return 0;
}
