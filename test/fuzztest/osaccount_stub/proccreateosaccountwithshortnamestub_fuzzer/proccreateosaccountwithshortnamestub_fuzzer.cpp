...
bool ProcCreateOsAccountWithShortNameStubFuzzTest(const uint8_t *data, size_t size)
{
    ...
    std::string localName(reinterpret_cast<const char *>(data), size);
    std::string shortName(reinterpret_cast<const char *>(data), size);

    MessageParcel datas;
    datas.WriteInterfaceToken(IOS_ACCOUNT_DESCRIPTOR);

    if (!datas.WriteString(localName)) {
        return false;
    }
    if (!datas.WriteString(shortName)) {
        return false;
    }
    ...
    osAccountManagerService_ ->OnRemoteRequest(
        static_cast<int32_t>(OsAccountInterfaceCode::CREATE_OS_ACCOUNT_WITH_SHORT_NAME), datas, reply, option);
