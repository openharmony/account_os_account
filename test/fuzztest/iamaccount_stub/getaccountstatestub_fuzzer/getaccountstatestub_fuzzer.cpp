...
#include "getaccountstatestub_fuzzer.h"
...
bool GetAccountStateStubFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size == 0)) {
        return false;
    }
    FuzzData fuzzData(data, size);
    int32_t userId = fuzzData.GetData<bool>() ?
        fuzzData.GetData<int32_t>() % Constants::MAX_USER_ID : fuzzData.GetData<int32_t>();
    ...
    uint32_t code = static_cast<uint32_t>(IAccountIAMIpcCode::COMMAND_GET_ACCOUNT_STATE);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::GetAccountStateStubFuzzTest(data, size);
    return 0;
}
