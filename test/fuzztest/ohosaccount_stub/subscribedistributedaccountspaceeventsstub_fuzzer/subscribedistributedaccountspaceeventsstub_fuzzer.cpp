...
#include "subscribedistributedaccountspaceeventsstub_fuzzer.h"
...
namespace OHOS {
bool SubscribeDistributedAccountSpaceEventsStubFuzzTest(const uint8_t* data, size_t size)
{
    ...
    accountManagerService->OnRemoteRequest(
        static_cast<int32_t>(IOsAccountSubProfileIpcCode::COMMAND_SUBSCRIBE_OS_ACCOUNT_SUB_PROFILE_EVENTS),
        datas, reply, option);
    return true;
}
}
...
void NativeTokenGet()
{
    ...
    infoInstance.processName = "SUBSCRIBE_OSACCOUNT_SPACE";
...
{
    OHOS::SubscribeDistributedAccountSpaceEventsStubFuzzTest(data, size);
    return 0;
}