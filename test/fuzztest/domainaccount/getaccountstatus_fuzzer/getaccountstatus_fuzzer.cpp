...
#include "getaccountstatus_fuzzer.h"
...
bool GetAccountStatusFuzzTest(const uint8_t* data, size_t size)
{
    bool ret = true;
    ...
    ret = DomainAccountClient::GetInstance().GetAccountStatus(info, status);
    return ret == ERR_OK;
}
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::GetAccountStatusFuzzTest(data, size);
    return 0;
}
