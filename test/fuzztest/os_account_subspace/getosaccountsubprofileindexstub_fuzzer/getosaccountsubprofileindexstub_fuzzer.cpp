...
#include "getosaccountsubprofileindexstub_fuzzer.h"
...
ErrCode OhosAccountManager::GetOsAccountSubProfileIndex(
...
bool GetOsAccountSubProfileIndexStubFuzzTest(const uint8_t* data, size_t size)
{
    ...
    if (!dataTemp.WriteInt32(osAccountId) || !dataTemp.WriteInt32(subProfileId)) {
        return false;
    }
    uint32_t code = static_cast<uint32_t>(
        IAccountIpcCode::COMMAND_GET_OS_ACCOUNT_SUB_PROFILE_INDEX);
...
void NativeTokenGet()
{
    ...
    infoInstance.processName = "GetOsAccountSubProfileIndexStub";
...
{
    OHOS::GetOsAccountSubProfileIndexStubFuzzTest(data, size);
    return 0;
}
