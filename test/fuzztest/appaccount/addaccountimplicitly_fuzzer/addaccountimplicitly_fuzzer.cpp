...
namespace OHOS {
    bool AddAccountImplicitlyFuzzTest(const uint8_t* data, size_t size)
    {
        bool result = false;
        if (size > 0) {
            ...
            result = AppAccountManager::AddAccountImplicitly(testOwner, testAuthType, options, callback);
        }
        return result;
    }
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::AddAccountImplicitlyFuzzTest(data, size);
    return 0;
}
