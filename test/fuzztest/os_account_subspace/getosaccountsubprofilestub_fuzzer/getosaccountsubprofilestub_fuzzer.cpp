...
#include "getosaccountsubprofilestub_fuzzer.h"
...
ErrCode OhosAccountManager::GetOsAccountSubProfile(
    int32_t id, int32_t subProfileId, OsAccountSubspaceResult &result, OhosAccountInfo &info)
{
    return ERR_OK;
}
}  // namespace AccountSA
...
bool GetOsAccountSubProfileStubFuzzTest(const uint8_t* data, size_t size)
{
    ...
    if (!dataFirst.WriteInt32(subProfileId)) {
        return false;
    }
    uint32_t code = static_cast<uint32_t>(IAccountIpcCode::COMMAND_GET_OS_ACCOUNT_SUB_PROFILE);
    ...
    subProfileId = fuzzData.GetData<int32_t>();
    if (!dataSecond.WriteInt32(osAccountId) || !dataSecond.WriteInt32(subProfileId)) {
        return false;
    }
    code = static_cast<uint32_t>(IAccountIpcCode::
        COMMAND_GET_OS_ACCOUNT_SUB_PROFILE_IN_INT_IN_INT_OUT_OSACCOUNTSUBSPACERESULT_OUT_OHOSACCOUNTINFO);
...
void NativeTokenGet()
{
    ...
    infoInstance.processName = "GetOsAccountSubProfileStubFuzzTest";
...
{
    OHOS::GetOsAccountSubProfileStubFuzzTest(data, size);
    return 0;
}
