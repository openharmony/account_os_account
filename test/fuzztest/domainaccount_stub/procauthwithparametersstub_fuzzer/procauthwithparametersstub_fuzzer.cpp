...
#include "procauthwithparametersstub_fuzzer.h"
...
public:
    ...
    bool ProcAuthWithParametersStubFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        info.status_ = static_cast<DomainAccountStatus>(fuzzData.GetData<int32_t>() % LOG_END);
        ...
        authOptions.serverParams_ = fuzzData.GenerateString();
        authOptions.hasServerParams_ = fuzzData.GenerateBool();
        if (!dataTemp.WriteParcelable(&authOptions)) {
            return false;
        }
        ...
        uint32_t code = static_cast<uint32_t>(IDomainAccountIpcCode::COMMAND_AUTH_WITH_PARAMETERS);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ProcAuthWithParametersStubFuzzTest(data, size);
    return 0;
}
