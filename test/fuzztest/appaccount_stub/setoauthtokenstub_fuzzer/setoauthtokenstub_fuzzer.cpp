...
#include "setoauthtokenstub_fuzzer.h"
...
bool SetOAuthTokenStubFuzzTest(const uint8_t* data, size_t size)
{
    ...
    std::string token = fuzzData.GenerateString();
    ...
    uint32_t code = static_cast<uint32_t>(IAppAccountIpcCode::COMMAND_SET_O_AUTH_TOKEN);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::SetOAuthTokenStubFuzzTest(data, size);
    return 0;
}
