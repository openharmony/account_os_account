...
#include "getosaccountlocalidbyserialnumber_fuzzer.h"
...
namespace OHOS {
    bool GetOsAccountLocalIdBySerialNumberFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        if ((data != nullptr) && (size != 0)) {
            ...
            int id = -1;
            result = OsAccountManager::GetOsAccountLocalIdBySerialNumber(testId, id);
        }
        return result;
    }
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::GetOsAccountLocalIdBySerialNumberFuzzTest(data, size);
    return 0;
}
