...
#include "authclientwithserver_fuzzer.h"
...
bool AuthWithServerFuzzTest(const uint8_t* data, size_t size)
{
    bool result = false;
    ...
    result = DomainAccountClient::GetInstance().Auth(info, password, authOptions, callback);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::AuthWithServerFuzzTest(data, size);
    return 0;
}
