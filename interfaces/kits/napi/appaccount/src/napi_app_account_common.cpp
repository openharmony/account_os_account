...
{
    ACCOUNT_LOGI("OnAccountsChanged start");
    ...
    uv_queue_work(loop,
        ...
        [](uv_work_t *work, int status) {
            ACCOUNT_LOGI("OnAccountsChanged uv_work_t start");
            if (work == nullptr) {
                return;
            }
    ...
    ACCOUNT_LOGI("OnAccountsChanged end");
}
...
void GetAppAccountInfoForResult(napi_env env, const std::vector<AppAccountInfo> &info,
    napi_value result)
{
    ACCOUNT_LOGI("enter");
...
void UnsubscribeCallbackCompletedCB(napi_env env, napi_status status, void *data)
{
    ...
    if (asyncContextForOff->argc >= UNSUBSCRIBE_MAX_PARA) {
        ...
        NAPI_CALL_RETURN_VOID(env, napi_call_function(env, undefined, callback, ARGS_SIZE_ONE,
            &results[0], &resultout));
    }
