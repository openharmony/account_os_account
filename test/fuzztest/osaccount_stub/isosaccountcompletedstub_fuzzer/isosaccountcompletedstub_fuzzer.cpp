...
#include "isosaccountcompletedstub_fuzzer.h"
#include <string>
#include <thread>
#include <vector>
...
bool IsOsAccountCompletedStubFuzzTest(const uint8_t *data, size_t size)
{
    ...
    osAccountManagerService_ ->OnRemoteRequest(
        static_cast<int32_t>(IOsAccountIpcCode::COMMAND_IS_OS_ACCOUNT_COMPLETED), datas, reply, option);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::IsOsAccountCompletedStubFuzzTest(data, size);
    return 0;
}
