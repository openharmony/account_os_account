...
#include "subscribeconstraints_fuzzer.h"
...
namespace OHOS {
    bool SubscribeConstraintsFuzzTest(const uint8_t* data, size_t size)
    {
        auto osSubscriber = std::make_shared<OsAccountSubscriber>();
        OsAccountManager::SubscribeOsAccount(osSubscriber);
        ...
        if ((data != nullptr) && (size != 0)) {
            ...
            result = OsAccountManager::SubscribeOsAccountConstraints(subscriber);
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::SubscribeConstraintsFuzzTest(data, size);
    return 0;
}
