...
#include "stoposaccountstub_fuzzer.h"
#include <string>
#include <thread>
#include <vector>
...
bool StopOsAccountStubFuzzTest(const uint8_t *data, size_t size)
{
    ...
    osAccountManagerService_ ->OnRemoteRequest(
        static_cast<int32_t>(OsAccountInterfaceCode::STOP_OS_ACCOUNT), datas, reply, option);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::StopOsAccountStubFuzzTest(data, size);
    return 0;
}
