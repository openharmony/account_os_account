...
#include "isosaccountdeactivatingstub_fuzzer.h"
#include "os_account_stub.h"
#include "account_log_wrapper.h"
#include "../common/fuzz_data.h"
...
namespace OHOS {
    bool IsOsAccountDeactivatingStubFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        if (!dataParcel.WriteInt32(localId)) {
            return false;
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::IsOsAccountDeactivatingStubFuzzTest(data, size);
    return 0;
}
