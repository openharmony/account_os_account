...
#include "getosaccountsubprofileidsstub_fuzzer.h"
...
bool GetOsAccountSubProfileIdsStubFuzzTest(const uint8_t* data, size_t size)
{
    ...
    uint32_t code = static_cast<uint32_t>(IAccountIpcCode::COMMAND_GET_OS_ACCOUNT_SUB_PROFILE_IDS);
    ...
    if (!dataSecond.WriteInt32(osAccountId)) {
        return false;
    }
    code = static_cast<uint32_t>(IAccountIpcCode::COMMAND_GET_OS_ACCOUNT_SUB_PROFILE_IDS_IN_INT_OUT_INT_VECTOR);
...
void NativeTokenGet()
{
    ...
    infoInstance.processName = "GetOsAccountSubProfileIdsStubFuzzTest";
...
{
    OHOS::GetOsAccountSubProfileIdsStubFuzzTest(data, size);
    return 0;
}
