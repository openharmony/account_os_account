...
#include "setaccountpolicy_fuzzer.h"
...
bool SetAccountPolicyFuzzTest(const uint8_t* data, size_t size)
{
    bool result = false;
    ...
    auto policy = fuzzData.GenerateString();
    result = DomainAccountClient::GetInstance().SetAccountPolicy(info, policy);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::SetAccountPolicyFuzzTest(data, size);
    return 0;
}
