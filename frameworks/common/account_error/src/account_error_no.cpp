...
{
    if ((errCode >= ERR_APPACCOUNT_KIT_NAME_IS_EMPTY && errCode <= ERR_APPACCOUNT_KIT_AUTH_TYPE_OUT_OF_RANGE) ||
    ...
    } else if (errCode == ERR_APPACCOUNT_SERVICE_GET_BUNDLE_NAME || errCode == ERR_APPACCOUNT_SERVICE_GET_BUNDLE_INFO) {
...
{
    return (errCode >= ERR_APPACCOUNT_KIT_NAME_IS_EMPTY && errCode <= ERR_APPACCOUNT_KIT_SEND_REQUEST);
}

static bool IsAppAccountServiceError(int32_t errCode)
{
    return (errCode >= ERR_APPACCOUNT_SERVICE_NAME_IS_EMPTY && errCode <= ERR_APPACCOUNT_SERVICE_WRITE_PARCEL);
}
