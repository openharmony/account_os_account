...
static napi_property_descriptor g_osAccountProperties[] = {
    ...
    DECLARE_NAPI_FUNCTION("getOsAccountNameByLocalId", GetOsAccountNameById),
...
{
    auto context = std::make_unique<GetOsAccountNameByIdContext>();
    context->env = env;
    context->throwErr = true;

    if (!ParseCallbackAndId(env, cbInfo, context->callbackRef, context->id, context->throwErr)) {
        return nullptr;
    }
    ...
    napi_value resource = nullptr;
    NAPI_CALL(env, napi_create_string_utf8(env, "GetOsAccountNameById", NAPI_AUTO_LENGTH, &resource));

    NAPI_CALL(env, napi_create_async_work(env, nullptr, resource, GetOsAccountNameByIdExecuteCB,
        GetOsAccountNameByIdCallbackCompletedCB,
        reinterpret_cast<void *>(context.get()), &context->work));
...
static bool IsSubscribeInVec(napi_env env, SubscribeCBInfo *subscribeCBInfo)
{
...
napi_value Subscribe(napi_env env, napi_callback_info cbInfo)
{
    ...
    if (IsSubscribeInVec(env, subscribeCBInfo)) {
        delete subscribeCBInfo;
    ...
    if (errCode != ERR_OK) {
        ...
        std::lock_guard<std::mutex> lock(g_lockForOsAccountSubscribers);
        g_osAccountSubscribers.emplace_back(subscribeCBInfo);
    }
    return WrapVoidToJS(env);
}
...
{
    auto bindDomainAccountContext = std::make_unique<BindDomainAccountAsyncContext>();
    bindDomainAccountContext->env = env;
    if (!ParseParaBindDomainAccount(env, cbInfo, bindDomainAccountContext)) {
        return nullptr;
    }

    napi_value result = nullptr;
    NAPI_CALL(env, napi_create_promise(env, &bindDomainAccountContext->deferred, &result));

    napi_value resource = nullptr;
    NAPI_CALL(env, napi_create_string_utf8(env, "BindDomainAccount", NAPI_AUTO_LENGTH, &resource));

    NAPI_CALL(env, napi_create_async_work(env, nullptr, resource,
        BindDomainAccountExecuteCB,
        BindDomainAccountCompletedCB,
        reinterpret_cast<void *>(bindDomainAccountContext.get()),
        &bindDomainAccountContext->work));
